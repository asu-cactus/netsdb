...
import time
import argparse
from utils import get_db_connection, create_tables, load_input_to_db, load_input_to_file_torch, load_kernel_to_file_torch, read_input_from_db, read_input_from_db_cx, read_all_input_from_db, read_all_input_from_db_cx
...
try:
    ...
    if load_data_from_file:
        for id in range(0, _iterations):
            inputs.append(torch.load(input_file_path + str(id) + '.pt'))
    elif db == 'postgres':
        inputs = read_all_input_from_db(db_cursor, input_dimensions)
        for id in range(0, _iterations):
            inputs[id] = torch.tensor(inputs[id], dtype=torch.float32) # TODO: find optimal implementation for this. 
    elif db == 'cx':
        inputs = read_all_input_from_db_cx(db_cursor, input_dimensions)
        for id in range(0, _iterations):
            inputs[id] = torch.tensor(inputs[id], dtype=torch.float32) # TODO: find optimal implementation for this.
    endTime = time.time()
    ...
    for id in range(0, _iterations):
        print ("input", inputs[id].shape)
        print ("input", inputs[id].dtype)

        startTime = time.time()
        output = torch.nn.functional.conv2d(inputs[id], filter, stride=stride, bias=bias)
        endTime = time.time()
        print(output)
