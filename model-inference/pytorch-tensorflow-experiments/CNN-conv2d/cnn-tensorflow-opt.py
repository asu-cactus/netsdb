...
import time
import argparse
from utils import get_db_connection, create_tables, load_input_to_db, load_input_to_file, load_kernel_to_file, read_kernel_data, read_input_from_db, read_all_input_from_db, read_input_from_db_cx, read_all_input_from_db_cx
...
try:
    ...
    if load_data_from_file:
        for id in range(0, _iterations):
            inputs.append(np.load(input_file_path + str(id) + '.npy'))
    elif db == 'postgres':
    ...
    for id in range(0, _iterations):
        ...
        startTime = time.time()
        output = tf.nn.conv2d(inputs[id], filter, strides=stride, padding='VALID')
        endTime = time.time()
        print(output)
