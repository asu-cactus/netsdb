...
def baseline_run_on_first_model():
    ...
    m1_ms = blocker.block_model_2d(m1, block_size_x=block_size_x, block_size_y=block_size_y,
                                   compute_magnitude=blocker.no_magnitude)
...
def baseline_run_on_second_model(method):
    ...
    if method == 'exact_match':
        # Run deduplication (Mistique exact match)
        print('Run deduplication with MISTIQUE: Exact Match')
        result_df = baseline.mistique_exact_match(m2, m2_ms, imdb_x_test, imdb_y_test, 
                                    blocks_indexer, stop_acc_drop=0.035)
    elif method == 'approximate_match':
        # Run deduplication (Mistique approximate match)
        print('Run deduplication with MISTIQUE: Approximate Match')
        blocks_indexer.discritized()
        result_df = baseline.mistique_approximate_match(m2, m2_ms, imdb_x_test, imdb_y_test, 
                                    blocks_indexer, stop_acc_drop=0.035)

    detector_output, num_unique_blocks = deduplicator.generate_detector_output(result_df, m2_ms, blocks_indexer.num_total)
    result_df.to_csv('baseline_imdb_embed_nontrainable_result.csv', index=False)
    np.save('baseline_imdb_embed_nontrainable_output.npy', detector_output)
...
def main():
    ...
    # Method: exact_match or approximate_match
    method = 'exact_match'

    baseline_run_on_first_model()
    baseline_run_on_second_model(method)
