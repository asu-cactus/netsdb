...
def run_on_model_1():
    """Here, we use the eurlex_4.3k_xml mdoel to build indexes
    ...
    # Build block indexes on eurlex_4.3k_xml
    m1 = tf.keras.models.load_model(os.path.join(model_path, 'eurlex_4.3k_xml.h5'))
    ...
    blocks_indexer.save('hetero_model_exp4_blocks_indexer_1.npy')
...
def run_on_model_2(method):
    ...
    # Create indexer
    blocks_indexer = indexer.Indexer(block_size_x=block_size_x, block_size_y=block_size_y)
    blocks_indexer.load('hetero_model_exp4_blocks_indexer_1.npy')
    ...
    result_df.to_csv('hetero_model_exp4_2.csv', index=False)
    np.save('hetero_model_exp4_2', detector_output)
