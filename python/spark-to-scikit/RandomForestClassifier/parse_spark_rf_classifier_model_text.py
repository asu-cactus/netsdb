...
def read_trees(trees_file):
    with open(trees_file) as treesfile:
        treesfile.readline() # ignore first line
...
def serialize_tree(root, left, right, features, thresholds, values, number_classes):
    ...
    if isinstance(name, str):
        ...
        values.append([0] * number_classes) # since we don't care about number of samples with belonging to threshold
    else:
        feature_num = -2
        threshold = -2.0
        class_name = int(name)
        value = [0] * number_classes
        value[class_name] = 1 # set value of the class to be 1, so this value is predicted 
