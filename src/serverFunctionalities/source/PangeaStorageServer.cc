...
void PangeaStorageServer::writeBackRecords(pair<std::string, std::string> databaseAndSet,
    ...
    std::cout << "to write back records for " << databaseAndSet.first << ":" 
        << databaseAndSet.second << std::endl;
    ...
    std::cout << numBytesToProcess << " bytes to write to a storage page" << std::endl;
    ...
    while (true) {
        ...
        try {
            ...
            while (allRecs.size() > 0) {
                ...
                std::cout << "to write " << numObjectsInRecord << " objects" << std::endl;
            ...
            bool fillNewPage = numBytesToProcess + (((numObjectsInRecord - pos) / numObjectsInRecord) *
                                     allRecs[allRecs.size() - 1]->numBytes()) > pageSize;
...
void PangeaStorageServer::registerHandlers(PDBServer& forMe) {
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<StorageAddSet>>([&](Handle<StorageAddSet> request,
            ...
            if (request->getPageSize() > conf->getMaxPageSize()) {
                ...
                std::cout << errMsg << std::endl;
                std::cout << "requested page size: " << request->getPageSize() << std::endl;
                std::cout << "max page size: " << conf->getMaxPageSize() << std::endl;
                res = false;
            } else {
                if (standalone == true) {
    ...
    // this handler accepts a request to add a set
    forMe.registerHandler(
        StorageAddIndexer_TYPEID,
        make_shared<SimpleRequestHandler<StorageAddIndexer>>([&](Handle<StorageAddIndexer> request,
                                                             PDBCommunicatorPtr sendUsingMe) {
            std::cout << "received StorageAddIndexer" << std::endl;
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<StorageClearSet>>([&](Handle<StorageClearSet> request,
            ...
            if (set == nullptr) {
                ...
                size_t desiredSize = set->getDesiredSize();
                LocalitySetReplacementPolicy policy = set->getReplacementPolicy();
                PersistenceType persistenceType = set->getPersistenceType();
                if (standalone == true) {
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<StorageAddData>>([&](Handle<StorageAddData> request,
                                                              PDBCommunicatorPtr sendUsingMe) {
            std::cout << "received StorageAddData" << std::endl;
            std::string errMsg;
            bool everythingOK = true;
            bool typeCheckOrNot = request->isTypeCheck();
            if (typeCheckOrNot == true) {
...
bool PangeaStorageServer::removeDatabase(std::string dbName) {
    ...
    if (it != this->dbs->end()) {
        ...
        dbs->erase(it);
        delete usersetSeqIds->at(dbName);
...
bool PangeaStorageServer::addSet(
    ...
    if (isMRU) {
        ...
        this->getCache()->pin(set, LRU, Write);
    }
...
Handle<AbstractIndexer>
PangeaStorageServer::getIndexerForType(std::string dbName,
                                       std::string typeName) {
  DatabaseID dbId = this->name2id->at(dbName);
  DefaultDatabasePtr db = this->getDatabase(dbId);
  UserTypeID typeId = this->typename2id->at(typeName);
...
Handle<AbstractIndexer>
PangeaStorageServer::getIndexerForType(std::string dbName, UserTypeID typeId) {
  DatabaseID dbId = this->name2id->at(dbName);
  DefaultDatabasePtr db = this->getDatabase(dbId);
  TypePtr type = db->getType(typeId);
...
Handle<AbstractIndexer>
PangeaStorageServer::getIndexerForType(DatabaseID dbId, UserTypeID typeId) {
  DefaultDatabasePtr db = this->getDatabase(dbId);
  TypePtr type = db->getType(typeId);
  return type->indexer;
}
        ...
        if (is_directory(root)) {
            ...
            for (iter = dbDirs.begin(); iter != dbDirs.end(); iter++) {
                if (is_directory(*iter)) {
                    ...
                    std::cout << "database name is " << name << std::endl;
...
void PangeaStorageServer::addDatabaseBySequenceFiles(string dbName, DatabaseID dbId, path dbPath) {
    ...
    // below paths will be freed in DefaultDatabase destructor
    vector<string>* dataDBPaths = new vector<string>();
...
void PangeaStorageServer::addDatabaseByPartitionedFiles(string dbName,
    ...
    if (this->dbs->find(dbId) != this->dbs->end()) {
        this->logger->writeLn("PDBStorage: database exists.");
        std::cout << "PDBStorage: database exists." << std::endl;
        return;
    }
    // create a database instance

    // below dataDBPaths will be freed in DefaultDatabase destructor
    vector<string>* dataDBPaths = new vector<string>();
    string dataDBPath;
    unsigned int i;
    ...
    // initialize it
    std::cout << "to initialize meta file: " << metaDBPath << std::endl;
    ...
    for (typeIter = types->begin(); typeIter != types->end(); typeIter++) {
        ...
        for (setIter = sets->begin(); setIter != sets->end(); setIter++) {
            ...
            std::cout << "Loaded existing set with database: " << dbName << ", type: " << typeName
    ...
    //below pointer will be removed when removing database
    SequenceID* seqId = new SequenceID();
