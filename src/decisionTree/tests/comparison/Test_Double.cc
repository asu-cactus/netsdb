...
// To compile the code and run it, just : $g++ -std=c++11 Test_Double.cc -o Test_Double
...
// $./Test_Double C ../higgs/higgs_0.txt ../higgs/higgs_1.txt
// $./Test_Double C ../higgs/higgs_0.txt ../higgs/higgs_1.txt ../higgs/higgs_2.txt ../higgs/higgs_3.txt ../higgs/higgs_4.txt ../higgs/higgs_5.txt ../higgs/higgs_6.txt ../higgs/higgs_7.txt ../higgs/higgs_8.txt ../higgs/higgs_9.txt
...
double most_common(double input_array[])
{
    int t;
    double most_value;
    int max = 1;
...
int main(int argc, const char *argv[]){
    ...
    double input[inNumRow][inNumFeature];
    // seed
    srand(time(NULL));
    for(int i = 0; i < inNumRow; i++){
		for(int j = 0; j < inNumFeature; j++){
            input[i][j] = rand() % (N + 1) / (double)(N + 1);
        }
    }

    // set the output array before voting
    double thisResultMatrix[inNumRow][numTrees];

    // set the final output array
    double resultMatrix[inNumRow];
    ...
    for (int i = 0; i < inNumRow; i++){
        ...
        double voteResult = most_common(thisResultMatrix[i]);
        resultMatrix[i] = voteResult;
    }
    auto end = std::chrono::high_resolution_clock::now();
    
    std::cout << "Double in C Inference Time: " << std::chrono::duration_cast<std::chrono::duration<float>>(end - begin).count() << " secs." << std::endl;
    return 0;
    /*
    for (int i = 0; i < inNumRow; i++){
