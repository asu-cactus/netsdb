...
def main():
    ...
    input = np.array([[0 for x in range(inNumFeature)] for y in range(inNumRow)], dtype='float64')

    for i in range(inNumRow):
        for j in range(inNumFeature):
            input[i][j] = np.float64(round(random.uniform(0.0, 1.0), 3))
    
    #print("finish generate the input matrix")
    # set the output array before voting
    thisResultMatrix = np.array([[0 for x in range(numTrees)] for y in range(inNumRow)], dtype='float64')

    # set the final output array
    resultMatrix = np.array([0 for _ in range(inNumRow)], dtype='float64')
