#ifndef SELFLEARNING_SERVER_H
#define SELFLEARNING_SERVER_H
...
// SelfLearningServer MUST co-locate with DistributedStorageManagerServer, QuerySchedulerServer and DispatcherServer.
// This server module collects information from other modules and store them into a SQLite database for automatic data placement.

// NOTE that all functions are not thread-safe.
// Because each time only one Job will be scheduled, so accesses to SelfLearningServer will be single-threaded.
class SelfLearningServer : public ServerFunctionality {
...
public:
    ...
    SelfLearningServer(   ConfigurationPtr conf,
                          int port);
    ...
    std::shared_ptr<SelfLearningDB> getDB();
    ...
    // to update the Job table for initial throughput field
    ...
    //to get latest lambda id for load job
    long getLatestLambdaIdForLoadJob (std::string loadJobName);
