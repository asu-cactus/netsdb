        ...
        if (sqlite3_prepare_v2(selfLearningDBHandler, queryString.c_str(), -1,
            &statement, NULL) == SQLITE_OK) {
             ...
             while ((res = sqlite3_step(statement)) == SQLITE_ROW) {
                 ...
                 float distance = sqlite3_column_int(statement, 1);
                 float selectivity = sqlite3_column_double(statement, 2)/(float)(1000000);
                 float numKeys = (float)(sqlite3_column_int64(statement, 3))/(float)(100000);
    ...
    if (sqlite3_prepare_v2(selfLearningDBHandler, queryString.c_str(), -1,
        ...
        if ((res = sqlite3_step(statement)) == SQLITE_ROW) {
            ...
            if (dataType == "Customer") {
                retState.inputSize = 2.44847 * (double)(dataScale)/1000.0;
            } else if (dataType == "LineItem") {
                retState.inputSize = 77.75727 * (double)(dataScale)/1000.0;
            } else if (dataType == "Nation") {
                retState.inputSize = 0.00002 * (double)(dataScale)/1000.0;
            } else if (dataType == "Orders") {
                retState.inputSize = 17.49195 * (double)(dataScale)/1000.0;
            } else if (dataType == "PartSupp") {
                retState.inputSize = 12.0485 * (double)(dataScale)/1000.0;
            } else if (dataType == "Part") {
                retState.inputSize = 2.43336 * (double)(dataScale)/1000.0;
            } else if (dataType == "Region") {
                retState.inputSize = 0.000003 * (double)(dataScale)/1000.0;
            } else if (dataType == "Supplier") {
                retState.inputSize = 0.14176 * (double)(dataScale)/1000.0;
            }
...
double getReward(const int& customerIndex, 
    ...
    sqlite3_finalize(statement);
    std::map<std::string, double> queryBaseLineLatency;
    queryBaseLineLatency["TPCHQuery01"] = 27;
    queryBaseLineLatency["TPCHQuery02"] = 58;
    queryBaseLineLatency["TPCHQuery04"] = 40;
    double totalLatency = 0.0;
    double baselineLatency = 0.0;
    for (auto query : querySet) {
        ...
        std::cout << "To get latency from run_stat for job <" << query.first
        ...
        if (sqlite3_prepare_v2(selfLearningDBHandler, queryString.c_str(), -1,
            ...
            if ((res = sqlite3_step(statement)) == SQLITE_ROW) {
                ...
                totalLatency += latency* (double)(query.second);
                baselineLatency += queryBaseLineLatency[query.first] * (double)(query.second);
                std::cout << "latency is " << latency << std::endl;
                std::cout << "totalLatency is " << totalLatency << std::endl;
                std::cout << "baselineLatency is " << baselineLatency << std::endl;
            } else {
                return -1;
            }
        } else {
    ...
    std::cout << "total latency for " << workloadSetSize << " workloads with dataScale = " << dataScale <<
                 " : " << totalLatency << std::endl; 
    double rewards = baselineLatency/totalLatency;
    std::cout << "to return reward = " << rewards;
    return rewards;
}
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::string> workloadSet = {"TPCHQuery01", "TPCHQuery02", "TPCHQuery04"};
    srand(time(NULL));
    double lastReward = 0;
    ...
    for (int iter = 0; iter < numIterations; iter++) {
        ...
        double sum = 0.0;
        for (int i = 0; i < workloadSet.size(); i++) {
            double curRatio = (double)(rand())/(double)(RAND_MAX);
            sum += curRatio;
            tempRatio.push_back(curRatio);
        }
        int total = 0;
        for (int i = 0; i < workloadSet.size()-1; i++) {
            int curCount = (int)((tempRatio[i]/sum)*(double)(curWorkloadSize)); 
            querySet[workloadSet[i]] = curCount;
            total += curCount;
            std::cout << "###########" << querySet[workloadSet[i]] << " " << workloadSet[i] << std::endl;
        }
        querySet[workloadSet[workloadSet.size()-1]] = curWorkloadSize - total;
        std::cout << "###########" << querySet[workloadSet[workloadSet.size()-1]] << " " << workloadSet[workloadSet.size()-1] << std::endl;
