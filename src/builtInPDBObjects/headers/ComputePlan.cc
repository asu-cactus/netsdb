...
// JiaNote: implemented following method to provide merger for partitioned join
inline SinkMergerPtr ComputePlan::getPartitionedMerger( int partitionId,
    ...
    SinkMergerPtr sinkMerger =
        myPlan->getNode(targetComputationName)
            .getComputation()
            .getSinkMerger(partitionId, targetSpec, targetAttsToOpOn, targetProjection, *this);
    return sinkMerger;
}

//to get merger for broadcast join
inline SinkMergerPtr ComputePlan::getMerger(std::string sourceTupleSetName,
                               std::string targetTupleSetName,
                               std::string targetComputationName) {

    return getPartitionedMerger(-1, sourceTupleSetName, targetTupleSetName, targetComputationName);
