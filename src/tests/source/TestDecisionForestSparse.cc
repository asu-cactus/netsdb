...
#include "EnsembleTreeGenericUDFSparse.h"
...
int main(int argc, char *argv[]) {
    ...
    if (createSet == false) {
        ff::createSetGeneric<float>(pdbClient, "decisionForest", "labels", "labels", 64);
        ...
        for (int i = 0; i < numPartitions; i++) {
            ...
            if (numPartitions == 1)
                inputScanner = pdb::makeObject<pdb::ScanUserSet<pdb::Map<int, float>>>("decisionForest", "inputs");
            else
                inputScanner = pdb::makeObject<pdb::ScanUserSet<pdb::Map<int, float>>>("decisionForest", std::string("inputs") + std::to_string(i));
            ...
            pdb::Handle<pdb::Computation> labelWriter = pdb::makeObject<pdb::WriteUserSet<float>>("decisionForest", "labels");

            labelWriter->setInput(decisionForestUDFSparse);
