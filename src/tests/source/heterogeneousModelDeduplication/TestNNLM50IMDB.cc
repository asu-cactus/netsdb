...
#ifndef TEST_NNLM50_IMDB_CC
#define TEST_NNLM50_IMDB_CC
...
int main(int argc, char *argv[]) {
    ...
    pdb::PDBLoggerPtr clientLogger =
        make_shared<pdb::PDBLogger>("NNLM50_IMDB_Log");
    ...
    if (loadData) {
        ff::createDatabase(pdbClient, "nnlm-50");
        ...
        //create input set
        pdbClient.createSet<FFMatrixBlock>("nnlm-50", "inputs", errMsg,
                     DEFAULT_PAGE_SIZE, "inputs", nullptr, nullptr, false);

        // create weights set 
        pdbClient.removeSet("nnlm-50", "weights", errMsg);
        pdbClient.createSet<FFMatrixBlock>("nnlm-50", "weights",
                                           errMsg, DEFAULT_PAGE_SIZE, "weights",
                                           nullptr, nullptr, false);
        // batch_size x vocab_size
        std::cout << "To load matrix for nnlm-50::inputs"
                  << std::endl;
        ff::loadMatrix(pdbClient, "nnlm-50", "inputs", batch_size,
                       vocab_size, block_x, block_y, false, false, errMsg);

        // embedding_dimension x vocab_size
        std::cout << "To load matrix for nnlm-50::weights"
                  << std::endl;
        ff::loadMatrix(pdbClient, "nnlm-50", "weights",
    ...
    // create output set
    pdbClient.removeSet("nnlm-50", "outputs", errMsg);
    pdbClient.createSet<FFMatrixBlock>("nnlm-50", "outputs", errMsg,
                                       DEFAULT_PAGE_SIZE, "outputs", nullptr,
                                       nullptr, false);
    // make the reader
    pdb::Handle<pdb::Computation> readA =
        makeObject<FFMatrixBlockScanner>("nnlm-50", "weights");
    pdb::Handle<pdb::Computation> readB =
        makeObject<FFMatrixBlockScanner>("nnlm-50", "inputs");
    ...
    myWriter =
        pdb::makeObject<FFMatrixWriter>("nnlm-50", "outputs");
    ...
    // verify the results
    ff::print_stats(pdbClient, "nnlm-50", "outputs");
    ff::print(pdbClient, "nnlm-50", "outputs");
}
#endif
