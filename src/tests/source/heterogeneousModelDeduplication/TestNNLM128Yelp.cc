...
int main(int argc, char *argv[]) {
    ...
    if (loadData) {
        ff::createDatabase(pdbClient, "nnlm-128");
        ...
        //create input set
        pdbClient.createSet<FFMatrixBlock>("nnlm-128", "inputs", errMsg,
                     DEFAULT_PAGE_SIZE, "inputs", nullptr, nullptr, false);

        // create weights set 
        pdbClient.removeSet("nnlm-128", "weights", errMsg);
        pdbClient.createSet<FFMatrixBlock>("nnlm-128", "weights",
                                           errMsg, DEFAULT_PAGE_SIZE, "weights",
                                           nullptr, nullptr, false);

        // batch_size x vocab_size
        std::cout << "To load matrix for nnlm-128::inputs"
                  << std::endl;
        ff::loadMatrix(pdbClient, "nnlm-128", "inputs", batch_size,
                       vocab_size, block_x, block_y, false, false, errMsg);

        // embedding_dimension x vocab_size
        std::cout << "To load matrix for nnlm-128::weights"
                  << std::endl;
        ff::loadMatrix(pdbClient, "nnlm-128", "weights",
    ...
    // create output set
    pdbClient.removeSet("nnlm-128", "outputs", errMsg);
    pdbClient.createSet<FFMatrixBlock>("nnlm-128", "outputs", errMsg,
                                       DEFAULT_PAGE_SIZE, "outputs", nullptr,
                                       nullptr, false);

    // make the reader
    pdb::Handle<pdb::Computation> readA =
        makeObject<FFMatrixBlockScanner>("nnlm-128", "weights");
    pdb::Handle<pdb::Computation> readB =
        makeObject<FFMatrixBlockScanner>("nnlm-128", "inputs");
    ...
    myWriter =
        pdb::makeObject<FFMatrixWriter>("nnlm-128", "outputs");
    ...
    // verify the results
    ff::print_stats(pdbClient, "nnlm-128", "outputs");
    ff::print(pdbClient, "nnlm-128", "outputs");
}
#endif
