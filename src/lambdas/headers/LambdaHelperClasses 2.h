...
public:
    ...
    // return the identifier of attribute or method name
    virtual std::string getIdentifierOfLambda() {
        return std::string("");
    }
    ...
    virtual std::string toTCAPString(std::vector<std::string>& inputTupleSetNames,
        ...
        outputTupleSetName = lambdaType + "_" + std::to_string(lambdaLabel) +
            tupleSetMidTag + computationName + std::to_string(computationLabel);

        outputColumnName = lambdaType + "_" + std::to_string(lambdaLabel) + "_" +
