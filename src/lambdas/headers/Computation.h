...
public:
    ...
    int getIndexInInputsByComputationName(std::string myComputationName) {
        for (int i = 0; i < inputs->size(); i++) {
            ...
            std::cout << "input " << i << "'s computation type is " 
                  << curComputation->getComputationType() << std::endl;
            std::cout << "input " << i << "'s computation name is " 
                  << curComputation->getComputationName() << std::endl;
