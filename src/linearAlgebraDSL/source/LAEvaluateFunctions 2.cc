...
pdb::Handle<pdb::Computation>& LAInitializerNode::evaluate(LAPDBInstance& instance) {
    ...
    int index = instance.getDispatchCount();
    std::string setName = "LA_" + method + "_" + std::to_string(index);
    instance.increaseDispatchCount();
    // now, create a new set in LA_db
    const UseTemporaryAllocationBlock tempBlock{1 * 1024 * 1024};
    Handle<LambdaIdentifier> myLambdaIdentifier = nullptr;
    /*if (index == 0) {
        myLambdaIdentifier = makeObject<LambdaIdentifier>(instance.getJobName()+"4", "JoinComp_2", "methodCall_0");
    } else if (index == 2){
        myLambdaIdentifier = makeObject<LambdaIdentifier>(instance.getJobName()+"4", "JoinComp_2", "methodCall_1");
    }*/
    if (!instance.getStorageClient().createSet<MatrixBlock>(
            "LA_db", setName, instance.instanceErrMsg(), size_t(64)*size_t(1024)*size_t(1024), "Load" + instance.getJobName() + std::to_string(index), nullptr, myLambdaIdentifier)) {
...
void LAStatementNode::evaluateQuery(LAPDBInstance& instance) {
    ...
    if (expression->isSyntaxSugarInitializer()) {
        ...
        if (statementQuery->getOutputType().compare("MatrixBlock") == 0) {
            if (!instance.getStorageClient().createSet<MatrixBlock>(
                    "LA_db", outputSetName, instance.instanceErrMsg(), size_t(64)*size_t(1024)*size_t(1024))) {
